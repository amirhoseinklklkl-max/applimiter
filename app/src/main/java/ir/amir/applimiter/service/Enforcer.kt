...
object Enforcer {
    ...
    fun evaluate(context: Context, pkg: String, tracker: UsageTracker) {
        ...
        val used = tracker.todayUsageFor(pkg)
    ...
    fun remainingMillis(context: Context, pkg: String, tracker: UsageTracker): Long? {
        val config = LimitStore.get(context, pkg) ?: return null
        val minutes = LimitStore.effectiveMinutes(config) ?: return 0L
        return (minutes * 60_000L - tracker.todayUsageFor(pkg)).coerceAtLeast(0L)
    }
