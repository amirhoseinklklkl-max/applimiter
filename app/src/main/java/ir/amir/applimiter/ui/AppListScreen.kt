...
fun AppListScreen(refreshKey: Int) {
    ...
    suspend fun reload(ctx: Context) {
        val loaded = withContext(Dispatchers.IO) { InstalledAppsRepository.load(ctx) }
        val u = withContext(Dispatchers.IO) { UsageTracker(ctx).todayUsageMillis(force = true) }
        val l = LimitStore.all(ctx).associateBy { it.packageName }
        apps = loaded
        usage = u
        limits = l
        loading = false
    ...
    LaunchedEffect(refreshKey) { reload(context) }
